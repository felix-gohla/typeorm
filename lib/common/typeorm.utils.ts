...
export function getDataSourceToken(
  ...
  return DEFAULT_DATA_SOURCE_NAME === dataSource
    ...
    : DEFAULT_DATA_SOURCE_NAME === getDataSourceName(dataSource)
    ? DataSource
...
export function getEntityManagerToken(
  ...
  return DEFAULT_DATA_SOURCE_NAME === dataSource
    ...
    : DEFAULT_DATA_SOURCE_NAME === getDataSourceName(dataSource)
    ? EntityManager
    : `${getDataSourceName(dataSource)}EntityManager`;
}
      ...
      retryWhen((e) =>
        e.pipe(
          scan((errorCount, error: Error) => {
            ...
            logger.error(
              `Unable to connect to the database${dataSourceInfo}.${verboseMessage} Retrying (${errorCount +
                1})...`,
              error.stack,
            );
...
export function getDataSourceName(dataSource: DataSourceOptions | DataSource): string {
  const options = dataSource instanceof DataSource
    ? dataSource.options
    : dataSource;
  return options.name ?? DEFAULT_DATA_SOURCE_NAME;
}
